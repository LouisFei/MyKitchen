...
Ext.define('MyKitchen.Component', {
    override: 'Ext.Component'
},
function (C) {
    var classRe = /^MyKitchen/,
        propRe = /^\$\{(\w+)\}$/,
        propsRe = /\$\{(\w+)\}/g,
        inherited = {
            'gray': 'classic',
    ...
    function alas (msg, className, prop) {
        ...
        msg = msg.replace('ZZ', MyKitchen.profileName);
    ...
    C.onExtended(function (D, classBody) {
        var className = classBody.$className,
            ...
            name = window.MyKitchen && MyKitchen.profileName,
            data, info;

        if (profiles) {
            for (; name; name = inherited[name]) {
                info = MyKitchen.mergeProfileInfo(profiles, name);
     ...
     *         info = Object.assign(info, profiles[deviceType].defaults, profiles[deviceType][buildProfile]);
     *     }
     *
     ...
     */
    Ext.ns('MyKitchen').mergeProfileInfo = function (profiles, name) {
        ...
        if (profiles[deviceType] && (profiles[deviceType] || profiles[deviceType].defaults)) {
